...
def raffle_allocation():
    ...
    hours = {}
    for log in logs:
        timestamp = datetime.datetime.strptime(log.request_time,
                                               "%Y-%m-%d %H:%M:%S")
        ...
        if log.url.endswith('add_ticket/'):
            hours[date][timestamp.hour]['add'] = hours[date][
                                                 timestamp.hour].get('add',
                                                                     0) + 1
        elif log.url.endswith('remove_ticket/'):
            hours[date][timestamp.hour]['remove'] = hours[date][
                                                    timestamp.hour].\
                                                    get('remove', 0) + 1
...
def raffle_users():
    ...
    for user in users:
        r1_tickets = user.raffleticket_set.filter(
            raffle_prize__deadline__round_name='Round 1').count()
        ...
        r2_points = ScoreboardEntry.objects.get(profile__user=user,
                                                round_name='Round 2').points
        r2_unused = max(((r1_points + r2_points) / 25) + r1_unused -
                        r2_tickets,
                        0)
        overall_unused = max((Profile.objects.get(
            user=user).points / 25) - r1_tickets - r2_tickets -
                             overall_tickets,
                             0)

        raffle_file.write('%s,%d,%d,%d,%d,%d,%d\n' % (
            user.username, r1_tickets, r1_unused, r2_tickets, r2_unused,
            overall_tickets, overall_unused))
