...
def index(request):
    ...
    if "widgets.energy_scoreboard" in settings.INSTALLED_WIDGET_APPS:
        module = importlib.import_module(
            "apps.widgets.energy_scoreboard.models")
...
def _get_layout(page_layouts, layout, css_style):
    """Fills in the page_layouts definitions from page_settings.py,
    Returns css style"""
    ...
    for row in layout[1]:
        ...
        for columns in row:
            if isinstance(columns,
                          types.TupleType):  # ((widget, 30%), (widget, 70%))
                widget = columns[0]
                gid = "%s" % (widget)
                ...
                if percent_total < 95:
                    layout_style += "#%s { float: left; width: %d%%; }" % (
                    gid, percent)
                else:
                    layout_style += "#%s { float: right; width: %d%%; }" % (
                    gid, percent)
                    percent_total = 0
                ...
                if layout[0] == "DEFAULT":
                    ...
                    widget_layout[
                    "template"] = "widgets/%s/templates/index.html" % widget
