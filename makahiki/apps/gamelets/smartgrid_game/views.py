...
from services.base_mgr import get_current_round
from gamelets.smartgrid_game.models import *
from gamelets.smartgrid_game import *
from services.team_mgr.models import *
from services.team_mgr import *
...
def __get_categories(user):
  ...
  if not categories:
      ...
      """
      cursor.execute('''SELECT  smartgrid_game_activitymember.activity_id,
            smartgrid_game_activitybase.slug as slug,
            smartgrid_game_commonactivityuser.approval_status,
            smartgrid_game_commonactivityuser.award_date
          FROM smartgrid_game_activitymember
          INNER JOIN smartgrid_game_commonactivityuser
            ON (smartgrid_game_activitymember.commonactivityuser_ptr_id = smartgrid_game_commonactivityuser.id)
          INNER JOIN smartgrid_game_ACTIVITYBASE
            ON (smartgrid_game_activitymember.activity_id = smartgrid_game_activitybase.id)
          WHERE smartgrid_game_activitymember.user_id = %s ''' % (user.id))

      activity_members = _dictfetchall(cursor)

      cursor.execute('''SELECT  smartgrid_game_commitmentmember.commitment_id,
            smartgrid_game_activitybase.slug as slug,
            smartgrid_game_commitmentmember.completion_date,
            smartgrid_game_commitmentmember.award_date
          FROM smartgrid_game_commitmentmember
          INNER JOIN smartgrid_game_ACTIVITYBASE
            ON (smartgrid_game_commitmentmember.commitment_id = smartgrid_game_activitybase.id)
          WHERE smartgrid_game_commitmentmember.user_id = %s ''' % (user.id))

      commitment_members = _dictfetchall(cursor)
      """
      cursor.execute('''SELECT smartgrid_game_activitybase.category_id,
            smartgrid_game_activitybase.id,
            smartgrid_game_activitybase.type,
            smartgrid_game_activitybase.depends_on,
            smartgrid_game_activitybase.depends_on_text,
          ...
          LEFT JOIN smartgrid_game_commitment
            ON (smartgrid_game_commitment.activitybase_ptr_id = smartgrid_game_activitybase.id)
          ORDER BY smartgrid_game_activitybase.category_id ASC, smartgrid_game_activitybase.priority ASC''')
