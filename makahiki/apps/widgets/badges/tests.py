...
from widgets.badges.management.commands.award_badge import award_badge
...
class DailyVisitorBadgeTest(TestCase):
    def test_awarding(self):
        ...
        self.assertEqual(user.badges_earned.all()[0].slug, "dailyvisitor",
...
class FullyCommittedBadgeTest(TestCase):
    def test_awarding(self):
        ...
        for index, commitment in enumerate(commitments):
            ...
            member.save()
            self.assertEqual(user.badges_earned.count(), 0,
                "Badge should not be awarded after commitment %i is awarded" % index)
            member.award_date = None
            member.save()

        self.assertEqual(user.badges_earned.count(), 1, "A badge should have been awarded.")
        self.assertEqual(user.badges_earned.all()[0].slug, "fully_committed",
