...
from widgets.smartgrid.models import ActivityBase, ActivityMember
        ...
        if not self.completed:
            ...
            for member in members:
                if member.activity not in user_activities:
                    new_member = ActivityMember.objects.create(user=self.user,
...
def mission_activity_handler(sender, instance=None, **kwargs):
    ...
    for mission in missions:
        if mission.is_completed(instance.user):
            member, _ = MissionMember.objects.get_or_create(user=instance.user,
                mission=mission)
