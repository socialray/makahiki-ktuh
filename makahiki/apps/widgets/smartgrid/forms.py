...
from widgets.smartgrid.models import ConfirmationCode, QuestionChoice, TextReminder
from widgets.smartgrid import get_user_by_email
...
class ActivityTextForm(forms.Form):
    ...
    def clean(self):
        ...
        if cleaned_data["question"] > 0:
            if (not cleaned_data.has_key("response")) and (
            not cleaned_data.has_key("choice_response")):
...
def _validate_social_email(self, cleaned_data):
    if self.activity.is_group and \
        (not cleaned_data.has_key("social_email") or \
         cleaned_data["social_email"] == None or \
         cleaned_data["social_email"] == ""):
        self._errors["social_email"] = ErrorList(["At least one email is required."])  # pylint: disable=W0212
...
def _validate_one_email(self, cleaned_data, email):
    if cleaned_data[email]:
        ...
        if user == None or user == self.request.user:
            self._errors[email] = ErrorList(["Invalid email. Please input only one valid email."])  # pylint: disable=W0212
