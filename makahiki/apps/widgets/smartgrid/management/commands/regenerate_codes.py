...
from apps.widgets.smartgrid.models import Activity, ConfirmationCode


class Command(management.base.BaseCommand):
    """command."""
    help = "Regenerate the confirmation codes for activities that use confirmation codes. " \
           "Takes optional parameters to delete the codes for an activity with the specified " \
           "slug(s)."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            'The confirmation codes for the following activities will be deleted and '
            'regenerated:\n')
    ...
    def _regenerate_codes(self, activities):
        ...
        for activity in activities:
            ...
            if not num_codes or num_codes == 0:
                self.stdout.write(
                    "ERROR: '%s' has event_max_seat of %d\n" % (activity.title, num_codes))
                return
