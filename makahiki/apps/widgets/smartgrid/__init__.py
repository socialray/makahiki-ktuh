...
from widgets.smartgrid.models import Activity, ActivityBase, Category, Commitment, ActivityMember, CommitmentMember
...
def get_available_golow_activities(user):
    ...
    activities = Activity.objects.exclude(
        ...
        expire_date__gte=datetime.date.today(),
        is_canopy=False,
    ).order_by("type", "priority").values("id", "depends_on", "type", "slug", "title", "point_value"
        , "point_range_end")
